...
Shopify.theme.jsSlideshowWithText = {
  init: function init($section) {
    ...
    var $iconSlideshowEl = $section.find('[data-icon-slideshow]').removeClass('is-hidden');
    ...
    var $iconSlideshow = $iconSlideshowEl.flickity({
      autoplay: false,
      contain: true,
      asNavFor: '[data-image-slideshow]',
      pageDots: false,
      draggable: false,
      prevNextButtons: false
      /*on: {
        ready: function ready() {
          var $currentIconSlide = $iconSlideshowEl.find('.is-selected .text-slideshow__content');
          Shopify.theme.animation.slideTransition($currentIconSlide, textTransition);
        }
      }*/
    });
    $imageSlideshow.on('change.flickity', function (event, index) {
     ...
     /* $iconSlideshow.flickity('select', index, true, true);
      var $currentIconSlide = $iconSlideshowEl.find('.is-selected .text-slideshow__content');
      Shopify.theme.animation.slideTransition($currentIconSlide, textTransition);*/
