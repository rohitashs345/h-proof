  ...
  updateQuantity: function ($el) {
    ...
    if ($el.data('update-quantity') === 'plus') {
      if ($el.data('limit') == 1) {
        ...
        quantityWarning.text('Limit: One per customer');
        quantityWarning.addClass('bounceIn');
        setTimeout(() => {
          quantityWarning.removeClass('bounceIn');          
        }, 1000);
        return;
      } else { 
...
function selectCallback(productEl, product, variant, state) {
  ...
  const priceServing = $('.js-serving-price');
  const priceSaleServing = $('.js-price-autodeliver');
  const priceVariantTotal = $('.js-variant-total');
  const priceSubsTotal = $('.js-subs-total');
  const priceBarTotal = $('.js-price-bar-total');
  const priceBarServing = $('.js-price-bar-serving');
  var dataSale = priceSaleServing.data('sale');
  var variantTitle = variant.title;
  var fixedPrice = variant.price;
  var divide = dataSale / 100;
  var dividePrice = fixedPrice * divide;
  var totalPriceSale = fixedPrice - dividePrice;
  let notifyMessage = '';
  ...
  if (variant) {
    ...
    if ($('.selling-plan-wrap.active').data('sell') == 'onetime') {
      var priceBarCal= fixedPrice;
      priceBarTotal.html(Shopify.formatMoney(fixedPrice, $('body').data('money-format')));
    } else {
      var priceBarCal =  totalPriceSale;
      priceBarTotal.html(Shopify.formatMoney(totalPriceSale, $('body').data('money-format')));
    }

    if(variantTitle.includes('20 Servings')) {
      var price = fixedPrice / 20;
      priceServing.html('(' + Shopify.formatMoney(price, $('body').data('money-format')) + ' / serving)');
      priceSaleServing.html('(' + Shopify.formatMoney(totalPriceSale / 20 , $('body').data('money-format')) + ' / serving)');
     
      priceBarServing.html(' (' + Shopify.formatMoney(priceBarCal / 20 , $('body').data('money-format')) + ' / serving)');
    } else if(variantTitle.includes('60 Servings')){
      var price = fixedPrice / 60;
      priceServing.html('(' + Shopify.formatMoney(price, $('body').data('money-format')) + ' / serving)');
      priceSaleServing.html('(' + Shopify.formatMoney(totalPriceSale / 60 , $('body').data('money-format')) + ' / serving)');

      priceBarServing.html(' (' + Shopify.formatMoney(priceBarCal / 60 , $('body').data('money-format')) + ' / serving)');
    }
    if(variantTitle.includes('120 Servings')){
      var price = fixedPrice / 120;
      priceServing.html('(' + Shopify.formatMoney(price, $('body').data('money-format')) + ' / serving)');
      priceSaleServing.html('(' + Shopify.formatMoney(totalPriceSale / 120 , $('body').data('money-format')) + ' / serving)');

      priceBarServing.html(' (' + Shopify.formatMoney(priceBarCal / 120 , $('body').data('money-format')) + ' / serving)');
    }
    if(variantTitle.includes('3 Tubes')){
      var price = fixedPrice / 15;
      priceServing.html('(' + Shopify.formatMoney(price, $('body').data('money-format')) + ' / serving)');
      priceSaleServing.html('(' + Shopify.formatMoney(totalPriceSale / 15 , $('body').data('money-format')) + ' / serving)');

      priceBarServing.html(' (' + Shopify.formatMoney(priceBarCal / 15 , $('body').data('money-format')) + ' / serving)');
    }
    if(variantTitle.includes('6 Tubes')){
      var price = fixedPrice / 30;
      priceServing.html('(' + Shopify.formatMoney(price, $('body').data('money-format')) + ' / serving)');
      priceSaleServing.html('(' + Shopify.formatMoney(totalPriceSale / 30 , $('body').data('money-format')) + ' / serving)');

      priceBarServing.html(' (' + Shopify.formatMoney(priceBarCal / 30 , $('body').data('money-format')) + ' / serving)');
    }
    if(variantTitle.includes('9 Tubes')){
      var price = fixedPrice / 45;
      priceServing.html('(' + Shopify.formatMoney(price, $('body').data('money-format')) + ' / serving)');
      priceSaleServing.html('(' + Shopify.formatMoney(totalPriceSale / 45 , $('body').data('money-format')) + ' / serving)');

      priceBarServing.html(' (' + Shopify.formatMoney(priceBarCal / 45 , $('body').data('money-format')) + ' / serving)');
    }

    function priceSaleSaving() {
      if(variant.compare_at_price > variant.price) {
        let amount = variant.compare_at_price - variant.price;
        $('.product-price__saving').text('YOU’RE SAVING '+Shopify.formatMoney(amount, $('body').data('money-format')))
      } else {
        $('.product-price__saving').text('');
      }
    }

    function priceSubSaving() {
      let percent = parseInt($('.js-subs-total').data('sale'));
      if(variant.compare_at_price > variant.price) {
        let amount = variant.compare_at_price - variant.price;
        let price_sub = variant.price *((100 - percent)/100);
        let price = amount * (percent / 100);
        let price_sale = amount + price;
        // console.log(price_sub);
        // console.log(variant.compare_at_price, product.price);
        $('.product-price__saving').text('YOU’RE SAVING '+Shopify.formatMoney(variant.compare_at_price - price_sub, $('body').data('money-format')));
        console.log(variant.compare_at_price, price_sub);
      } else {
        let price =  variant.price * ((100 - percent ) / 100);
        let amount = variant.price - price;
        $('.product-price__saving').text('YOU’RE SAVING '+Shopify.formatMoney(product.price_min - price, $('body').data('money-format')));
        if(variantTitle.includes('60')){
          $('.product-price__saving').text('YOU’RE SAVING '+Shopify.formatMoney((product.price_min * 3) - price, $('body').data('money-format')));
        }

        if(variantTitle.includes('120')){
          $('.product-price__saving').text('YOU’RE SAVING '+Shopify.formatMoney((product.price_min * 6) - price, $('body').data('money-format')));
        }
        // $('.product-price__saving').text('YOU’RE SAVING '+Shopify.formatMoney(amount, $('body').data('money-format')));
      }
    };
    ...
    if($('.subscription-wrap.active').length) {
      priceSubSaving();
    }

    $('body').on('click', '.subscription-wrap', function() {
      priceSubSaving();
    });

    $('body').on('click', '.onetime-wrap', function() {
      priceSaleSaving();
    })

    priceVariantTotal.html(Shopify.formatMoney(fixedPrice, $('body').data('money-format')));
    priceSubsTotal.html(Shopify.formatMoney(totalPriceSale, $('body').data('money-format')));
  } else {
...
function sellingPlan() {
  $('.js-selling-plan-block').on('click', function() {
    if (!$(this).parent().hasClass('active')) {
      $('.selling-plan-wrap').removeClass('active');
      $(this).parent().addClass('active');

      let checkOption = $(this).data('sell');
      if (checkOption == 'onetime') {
        $('.js-selling-plan-id').attr('name', '');
        $('shopify-payment-terms').show();

        $('.js-price-bar-total').html($('.js-variant-total').html());
        $('.js-price-bar-serving').html($('.js-serving-price').html());
      } else {
        $('.js-selling-plan-id').attr('name', 'selling_plan');
        $('shopify-payment-terms').hide();

        $('.js-price-bar-total').html($('.js-subs-total').html());
        $('.js-price-bar-serving').html($('.js-price-autodeliver').html());
      }
    } else if ($(this).data('sell') == 'subscription') {
      $('.js-selling-plan-block[data-sell="onetime"]').trigger('click');
    }
  });
}

var currentHandle = $('[data-option-index="0"] [data-variant-option-value]:checked').data('handle');
$('.swatch-metafield [data-swatch-label]').attr('data-swatch-label', currentHandle);

$('body').on('change', '[data-option-index="0"] [data-variant-option-value]', function() {
  var handle = $(this).data('handle');
  $('.swatch-metafield [data-swatch-label]').attr('data-swatch-label', handle);
})

$('body').on('click', '.three-block-mobile__item', function(e) {
  e.preventDefault();
  let data_id = $(this).data('id');
  $('.three-block-item').removeClass('active');
